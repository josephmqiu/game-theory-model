...
} from "../store/conversation"; // TODO: store import — needs PipelineHost
import { createAppStore, resetPipelineRuntimeStore } from "../store"; // TODO: store import — needs PipelineHost
import {
...
} from "../store/pipeline-runtime"; // TODO: store import — needs PipelineHost
import {
  addSteeringMessage,
  getPipelineState,
  resetPipelineStore,
...
} from "../store/pipeline"; // TODO: store import — needs PipelineHost
import type {
  EliminationResult,
  MetaCheckResult,
...
function createTestHarness() {
  ...
  const orchestrator = createPipelineOrchestrator({
    ...
    getAnalysisFile: () => {
      ...
      return meta
        ? storeToAnalysisFile(appStore.getState().canonical, meta)
        : null;
    },
    ...
    startPipelineAnalysis: (params) => startPipelineAnalysis(params),
    updateAnalysisState: (updater) => updateAnalysisState(updater),
    setPhaseResult: (phase, result) => setPhaseResult(phase, result),
    upsertPhaseExecution: (execution) => upsertPhaseExecution(execution),
    setPipelineProposalReview: (review) => setPipelineProposalReview(review),
    addSteeringMessage: (content) => {
      addSteeringMessage(content);
    },

    // Pipeline runtime
    getPipelineRuntimeState: () => getPipelineRuntimeState(),
    registerPendingRevalidationApproval: (approval) =>
      registerPendingRevalidationApproval(approval),
    clearPendingRevalidationApproval: (eventId) =>
      clearPendingRevalidationApproval(eventId),
    setActiveRerunCycle: (cycle) => setActiveRerunCycle(cycle),
    updatePromptRegistry: (updater) => updatePromptRegistry(updater),
...
async function seedPhase2Revalidation() {
  ...
  await harness.orchestrator.startAnalysis(
  ...
  const event = Object.values(
    harness.appStore.getState().canonical.revalidation_events,
  )[0];
  if (!event) {
...
describe("pipeline orchestrator revalidation flow", () => {
  ...
  it("creates a revalidation event and conversation card after phase 2 reveals internal agency", async () => {
    ...
    expect(getPipelineState().analysis_state?.phase_states[3]?.status).toBe(
      "needs_rerun",
    );
    expect(getPipelineState().analysis_state?.phase_states[4]?.status).toBe(
      "needs_rerun",
    );
    expect(
      getPipelineRuntimeState().pending_revalidation_approvals[event.id],
    ).toBeDefined();
    expect(
      getConversationState().messages.some(
        (message) => message.message_type === "revalidation",
      ),
    ).toBe(true);
    expect(
      appStore.getState().canonical.revalidation_events[event.id],
    ).toBeDefined();
  });
  ...
  it("approving a revalidation event increments the pass number and queues a rerun cycle", async () => {
    ...
    expect(getPipelineRuntimeState().active_rerun_cycle?.event_id).toBe(
      event.id,
    );
    expect(getPipelineRuntimeState().active_rerun_cycle?.earliest_phase).toBe(
      3,
    );
    expect(
      appStore.getState().canonical.revalidation_events[event.id]?.resolution,
    ).toBe("approved");
    expect(orchestrator.getPendingRevalidations()).toHaveLength(0);
    await expect(
      orchestrator.approveRevalidation(event.id),
    ).resolves.toBeNull();
  });
  ...
  it("keeps an approved rerun active until review clears, then marks it complete", async () => {
    ...
    const event = Object.values(
      appStore.getState().canonical.revalidation_events,
    )[0];
    if (!event) {
    ...
    expect(getPipelineRuntimeState().active_rerun_cycle?.status).toBe(
      "running",
    );
    expect(
      appStore.getState().canonical.revalidation_events[event.id]?.resolution,
  ...
  it("returns phase 5 as partial while pending revalidation or active rerun state exists", async () => {
    ...
    expect(getPipelineState().analysis_state?.phase_states[5]?.status).toBe(
      "review_needed",
    );
  });
  ...
  it("dismissing a revalidation event restores phase statuses while keeping stale warnings intact", async () => {
    ...
    expect(
      appStore.getState().canonical.games.game_revalidation.stale_markers
        ?.length,
    ).toBeGreaterThan(0);
  });
  ...
  it("preserves needs_rerun when dismissing one of multiple overlapping pending events", async () => {
    ...
    const [firstEvent, secondEvent] = Object.values(
      appStore.getState().canonical.revalidation_events,
    );
    ...
    expect(
      appStore.getState().canonical.revalidation_events[firstEvent.id]
        ?.resolution,
    ).toBe("dismissed");
    expect(
      appStore.getState().canonical.revalidation_events[secondEvent.id]
        ?.resolution,
    ).toBe("pending");
    expect(getPipelineState().analysis_state?.phase_states[3]?.status).toBe(
      "needs_rerun",
  ...
  it("runs phase 7 once phase 6 is complete and stores the extracted assumptions", async () => {
    ...
    expect(
      getConversationState().proposal_review.proposals.length,
    ).toBeGreaterThan(0);
  });
  ...
  it("creates a revalidation event when phase 7 finds a stale critical empirical assumption", async () => {
    ...
    const phase7 = getPipelineState().phase_results[7] as
      ...
      | undefined;
    expect(
      phase7?.assumptions.find(
        (assumption) => assumption.temp_id === "assumption_stale",
      )?.sensitivity,
    ).toBe("critical");
    const event = Object.values(
      appStore.getState().canonical.revalidation_events,
    )[0];
    expect(event?.trigger_condition).toBe(
  ...
  it("runs phase 8 elimination after prior phases are seeded", async () => {
    ...
    expect(result.status).toBe("complete");
    const phase8 = getPipelineState().phase_results[8] as
      | EliminationResult
      | undefined;
    expect(phase8).toBeDefined();
    expect(phase8?.eliminated_outcomes).toBeDefined();
    expect(getConversationState().messages.some((m) => m.phase === 8)).toBe(
      true,
    );
  });
  ...
  it("runs phase 9 scenario generation after phase 8 completes", async () => {
    ...
    expect(result.status).toBe("complete");
    const phase9 = getPipelineState().phase_results[9] as
      | ScenarioGenerationResult
      | undefined;
  ...
  it("runs phase 10 meta-check and reports analysis_complete", async () => {
    ...
    expect(result.status).toBe("complete");
    const phase10 = getPipelineState().phase_results[10] as
      | MetaCheckResult
      | undefined;
    ...
    expect(getConversationState().messages.some((m) => m.phase === 10)).toBe(
      true,
    );
  });
});
