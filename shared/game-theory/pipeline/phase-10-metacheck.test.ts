...
describe("Phase 10 meta-check runner", () => {
  ...
  it("answers all 10 meta-check questions with concern levels", () => {
    ...
    const { phase4, phase6, phase7, phase8, phase9 } =
      runPriorPhases(canonical);
    ...
    const questionNumbers = result.meta_check_answers.map(
      (a) => a.question_number,
    );
  ...
  it("answers all 6 final test questions with completeness", () => {
    ...
    const questionNumbers = result.final_test_answers.map(
      (a) => a.question_number,
    );
  ...
  it("runs adversarial challenge with severity ratings", () => {
    ...
    expect(validAssessments).toContain(
      result.adversarial_result.overall_assessment,
    );
  });
