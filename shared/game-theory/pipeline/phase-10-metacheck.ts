...
function answerQ1LeastAnalyzedPlayer(
  ...
  MetaCheckAnswer,
  "answer" | "concern_level" | "revision_triggered" | "evidence_refs"
> {
...
> {
  if (!phase9 || !phase7) {
    return {
      answer:
        "Central thesis or assumptions not available; cannot identify counterargument.",
      concern_level: "minor",
      revision_triggered: null,
      evidence_refs: [],
    };
  ...
  const criticalInferenceOnly = phase7.assumptions.filter(
    (a) =>
      a.sensitivity === "critical" && a.evidence_quality !== "direct_evidence",
  );

  if (criticalInferenceOnly.length === 0) {
    return {
      answer:
        "No critical assumptions rely solely on inference; central thesis is well-grounded in evidence.",
      concern_level: "none",
      revision_triggered: null,
      evidence_refs: [],
    };
...
> {
  ...
  const assumptionOnly = phase7.assumptions.filter(
    (a) =>
      a.sensitivity === "critical" && a.evidence_quality === "assumption_only",
  );
  ...
  if (assumptionOnly.length === 0) {
    ...
    if (inferenceOnly.length === 0) {
      return {
        answer:
          "All critical assumptions have direct evidence. No single piece of information would dramatically shift predictions.",
        concern_level: "none",
        revision_triggered: null,
        evidence_refs: [],
      };
...
> {
  ...
  if (signalCount === 0) {
    return {
      answer:
        "No signal classifications in the model to assess statement informativeness.",
      concern_level: "none",
      revision_triggered: null,
      evidence_refs: [],
    };
...
> {
  ...
  if (gameCount <= 1 && playerCount <= 2) {
    return {
      answer:
        "The current model is already minimal (1 game, 2 or fewer players).",
      concern_level: "none",
      revision_triggered: null,
      evidence_refs: [],
    };
...
> {
  if (!phase6 || !phase6.behavioral_overlays) {
    return {
      answer:
        "No adjacent analytical tools (behavioral overlays) were applied in this analysis.",
      concern_level: "none",
      revision_triggered: null,
      evidence_refs: [],
    };
  ...
  if (overlays.length === 0) {
    return {
      answer:
        "Behavioral overlay section exists but no overlays were identified.",
      concern_level: "none",
      revision_triggered: null,
      evidence_refs: [],
    };
...
function answerFT1SmallestGame(
  ...
  if (games.length === 0) {
    return {
      ...
      gap_description:
        "At least one strategic game is required to identify the core tension.",
    };
  }
...
function answerFT2RepeatedHistory(
  ...
  if (!hasPhase4History(phaseResults)) {
    return {
      ...
      gap_description:
        "Phase 4 repeated-game history analysis was not run or not provided.",
    };
  }

  return {
    answer:
      "Phase 4 history data exists. Repeated interaction patterns, trust assessments, and dynamic inconsistency risks have been mapped into the model, shaping current incentive and belief structures.",
    completeness: "fully_answered",
    gap_description: null,
  };
}
...
function answerFT3FeasibleStrategies(
  ...
  const gamesWithConstraints = Object.values(canonical.games).filter(
    (game) =>
      game.institutional_constraints &&
      game.institutional_constraints.length > 0,
  );
  ...
  if (gamesWithConstraints.length === 0) {
    ...
    const allStrategies = Object.values(canonical.formalizations).flatMap(
      (f) =>
        f.kind === "normal_form"
          ? Object.entries(f.strategies).flatMap(([playerId, strategies]) =>
              strategies.map(
                (s: string) =>
                  `${canonical.players[playerId]?.name ?? playerId}: ${s}`,
              ),
            )
          : [],
    );
    ...
    return {
      ...
      completeness: "partially_answered",
      gap_description:
        "No institutional constraints have been added to any game.",
    };
  }
  ...
  const constraintSummary = gamesWithConstraints
    .map(
      (g) =>
        `"${g.name}": ${g.institutional_constraints!.length} constraint(s)`,
    )
    .join("; ");
...
function answerFT4Equilibrium(
  ...
  if (!phase6) {
    return {
      ...
      completeness: "partially_answered",
      gap_description:
        "Equilibrium analysis requires Phase 6 formalization results.",
    };
  }

  const analyses = phase6.baseline_equilibria.analyses;
  const successCount = analyses.filter((a) =>
    a.solver_summaries.some((s) => s.status === "success"),
  ).length;
  ...
  const totalEquilibria = analyses.reduce(
    (sum, a) =>
      sum +
      a.solver_summaries.reduce(
        (s, solver) => s + (solver.equilibrium_count ?? 0),
        0,
      ),
    0,
  );
  ...
  return {
    ...
    gap_description:
      successCount === 0
        ? "No solvers succeeded in computing equilibria."
        : null,
  };
}
...
function answerFT5ModelVsJudgment(
  ...
  const equilibriumCount = phase9.proposed_scenarios.filter(
    (s) => s.forecast_basis === "equilibrium",
  ).length;
  const discretionaryCount = phase9.proposed_scenarios.filter(
    (s) => s.forecast_basis === "discretionary",
  ).length;
  const mixedCount = phase9.proposed_scenarios.filter(
    (s) => s.forecast_basis === "mixed",
  ).length;
...
function answerFT6EvidenceThatWouldChange(
  ...
  if (phase9) {
    conditions.push(
      `Thesis falsification: ${phase9.central_thesis.falsification_condition}`,
    );
  }

  if (phase7) {
    const critical = phase7.assumptions.filter(
      (a) => a.sensitivity === "critical",
    );
    for (const assumption of critical.slice(0, 3)) {
      conditions.push(
        `If "${assumption.statement}" is falsified: ${assumption.what_if_wrong}`,
      );
    }
  }
  ...
  if (conditions.length === 0) {
    return {
      ...
      completeness: "partially_answered",
      gap_description:
        "Phase 7 and Phase 9 data needed to identify prediction-changing evidence.",
    };
  }
...
function buildAdversarialChallenges(
  canonical: CanonicalStore,
  _phase6: FormalizationResult | null,
  phase7: AssumptionExtractionResult | null,
  phase9: ScenarioGenerationResult | null,
): AdversarialChallenge[] {
  ...
  if (phase7) {
    const weakCritical = phase7.assumptions.filter(
      (a) =>
        ...
        (a.evidence_quality === "inference" ||
          a.evidence_quality === "assumption_only"),
    );
  ...
  if (phase7 && phase9) {
    ...
    if (clusterIds.size > 0) {
      ...
      if (
        referencedClusterAssumptions.length === 0 &&
        clusteredAssumptionIds.size > 0
      ) {
        challenges.push(
          makeChallenge(
            "naive_independence",
...
): AdversarialResult["overall_assessment"] {
...
function collectRevisionsTriggers(
  ...
  for (const answer of metaCheckAnswers) {
    if (
      answer.revision_triggered !== null &&
      !seen.has(answer.revision_triggered)
    ) {
...
export function runPhase10MetaCheck(
  ...
  const analysisComplete = determineCompleteness(
    metaCheckAnswers,
    finalTestAnswers,
  );
