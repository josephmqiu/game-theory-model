...
const SUPPORTED_PHASE6_FORMALIZATION_KINDS = new Set<
...
function createProposalGroupRecord(): Record<
  Phase6Subsection,
  ModelProposal[]
> {
  return {
...
function buildAssumptionCommand(
  id: string,
  statement: string,
  type:
    | "behavioral"
...
function planFormalRepresentations(
  context: Phase6RunnerContext,
  state: Phase6WorkingState,
): FormalRepresentationResult {
  const game = firstGame(context.canonical);
