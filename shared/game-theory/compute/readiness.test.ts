...
describe("computeReadiness", () => {
  ...
  it("returns not_ready when payoff cells are missing", () => {
    ...
    expect(report.readiness.blockers.join(" ")).toContain(
      "Missing payoff cells",
    );
  });
  ...
  it("warns on stale assumptions and ordinal-only Nash inputs", () => {
    ...
    expect(report.per_solver.nash?.blockers.join(" ")).toContain(
      "Mixed-strategy Nash requires cardinal payoffs",
    );
  });
});
