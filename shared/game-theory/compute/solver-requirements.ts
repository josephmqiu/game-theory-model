...
function confidenceFloorForFormalization(
  ...
  const confidences = collectRelevantEstimates(formalization, store).map(
...
function normalFormBaseGate(
  ...
  if (players.length > 2) {
    blockers.push(
      "N-player equilibrium search is deferred. Use a 2-player formalization for M4 solvers.",
    );
  }
  ...
  return {
    ...
    warnings: [],
    completeness:
      coverage.expected === 0 ? 0 : coverage.actual / coverage.expected,
    confidenceFloor,
  };
}
...
function checkBackwardInduction(
  formalization: Formalization,
  store: CanonicalStore,
): SolverGateResult {
  if (formalization.kind !== "extensive_form") {
    return createGateResult(
      false,
      ["Backward induction requires an extensive-form formalization."],
      [],
      0,
    );
  }
  ...
  if (ef.root_node_id) {
    const cycleNodeId = findReachableCycleNode(
      ef.root_node_id,
      buildOutgoingEdgeMap(edges),
    );
    if (cycleNodeId) {
  ...
  const leaves = nodes.filter(
    (node) => (outgoingByNode.get(node.id) ?? 0) === 0,
  );
  if (leaves.length === 0) {
  ...
  for (const informationSet of ef.information_sets) {
    if (informationSet.node_ids.length < 2) {
      warnings.push(
        `Information set ${informationSet.id} contains a single node.`,
      );
    }
  }
  ...
  return createGateResult(
    ...
    nodes.length === 0
      ? 0
      : (nodes.length - leavesWithoutPayoffs.length) / nodes.length,
    confidenceFloorForFormalization(formalization, store),
  );
}

function checkBayesianUpdate(
  formalization: Formalization,
  store: CanonicalStore,
): SolverGateResult {
  if (formalization.kind !== "bayesian") {
    return createGateResult(
      false,
      ["Bayesian update requires a Bayesian formalization."],
      [],
      0,
    );
  }
  ...
  if (
    !bayesian.signal_structure ||
    bayesian.signal_structure.signals.length === 0
  ) {
...
export const solverRequirementTable: Record<SolverKind, SolverRequirement> = {
  ...
  simulation: {
    ...
    check: (formalization, store) => {
      ...
      const eligible =
        nodes > 0 && edges > 0 && (game?.players.length ?? 0) > 0;
      return createGateResult(
        eligible,
        eligible
          ? []
          : [
              "Simulation requires nodes, edges, and at least one assigned player.",
            ],
        [],
        eligible ? 1 : 0.4,
        confidenceFloorForFormalization(formalization, store),
  ...
  correlated_equilibrium: {
    ...
    check: () =>
      notImplementedGate("Correlated equilibrium solver not yet implemented."),
  },
  credible_commitment: {
    solver: "credible_commitment",
    check: () =>
      notImplementedGate("Credible commitment solver not yet implemented."),
  },
  game_classifier: {
  ...
  mechanism_design: {
    ...
    check: () =>
      notImplementedGate("Mechanism design solver not yet implemented."),
  },
};
...
export function candidateSolversForFormalization(
  formalization: Formalization,
): SolverKind[] {
  switch (formalization.kind) {
    ...
    case "coalition":
      return ["cascade"];
    default:
      return solverKinds.filter(
        (solver) => solver !== "cascade" && solver !== "simulation",
      );
  }
}
