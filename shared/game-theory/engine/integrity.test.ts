...
import {
  buildInverseIndex,
  refreshInverseIndexForEntities,
} from "./inverse-index";
...
describe("integrity and cascade behavior", () => {
  ...
  it("cascades source deletion through observations and stale-marks emptied claims with the deleted dependency as cause", () => {
    ...
    expect(
      result.store.claims.claim_1.stale_markers?.some(
        (marker) =>
          marker.caused_by.type === "observation" &&
          marker.caused_by.id === "observation_1",
      ),
    ).toBe(true);
  });
  ...
  it("removes a deleted player from a game and marks the game stale if its roster empties", () => {
    ...
    expect(
      result.store.games.game_1.stale_markers?.some(
        (marker) =>
          marker.caused_by.type === "player" &&
          marker.caused_by.id === "player_1",
      ),
    ).toBe(true);
  });
  ...
  it("computes impact reports and indexes wildcard references", () => {
    ...
    const impact = computeImpact(
      store,
      index,
      createEntityRef("source", "source_1"),
    );
    expect(impact.direct_dependents).toContainEqual(
      createEntityRef("observation", "observation_1"),
    );
  ...
  it("marks scenarios stale when a referenced edge is deleted and updates the inverse index incrementally", () => {
    ...
    const newClaimId = Object.keys(addResult.store.claims).find(
      (id) => id !== "claim_1",
    )!;
    ...
    expect(
      refreshed[refKey(createEntityRef("observation", "observation_1"))],
    ).toContainEqual(createEntityRef("claim", newClaimId));
  ...
  it("cascades game deletion through formalizations, nodes, and edges while stale-marking external scenarios", () => {
    ...
    store.formalizations.formalization_2 = {
      ...
      assumptions: [],
      notes:
        "External formalization that still references the first game edge.",
    };
  ...
  it("expands delete cascades into atomic batch commands without encoding stale fallout as explicit mark_stale commands", () => {
    const store = createSampleCanonicalStore();
    const expanded = expandCascade(store, buildInverseIndex(store), {
      kind: "delete_source",
      payload: { id: "source_1" },
    });
    ...
    expect(
      expanded.commands.some(
        (command) => command.kind === "delete_observation",
      ),
    ).toBe(true);
    expect(
      expanded.commands.some((command) => command.kind === "mark_stale"),
    ).toBe(false);
  });
});
