...
const phaseDefinitions = {
  1: {
    ...
    description:
      "Phase 1: Situational Grounding. Research the strategic situation described by the user and produce evidence proposals across the 7 evidence categories.",
    schema: z
      .object({
  ...
  2: {
    ...
    description:
      "Phase 2: Identify strategic players, objectives, and information asymmetries. Requires Phase 1 to be complete.",
    schema: z
      .object({
  ...
  3: {
    ...
    description:
      "Phase 3: Build the smallest baseline strategic model and canonical game mapping. Requires Phase 2 to be complete.",
  ...
  4: {
    ...
    description:
      "Phase 4: Map historical interactions, trust, and repeated-game patterns. Requires Phase 3 to be complete.",
    schema: z.object({}).strict(),
  },
  6: {
    ...
    description:
      "Phase 6: Run full formalization across subsections 6a-6i, using proposal-driven canonical updates and derived solver summaries.",
    schema: z
      .object({
        subsections: z
          .array(z.enum(["6a", "6b", "6c", "6d", "6e", "6f", "6g", "6h", "6i"]))
          .optional(),
      })
      .strict(),
  },
  7: {
    ...
    description:
      "Phase 7: Extract assumptions, classify empirical versus game-theoretic dependencies, rate sensitivity, and identify correlated clusters.",
    schema: z.object({}).strict(),
  },
  8: {
    ...
    description:
      "Phase 8: Eliminate implausible outcomes with specific phase citations.",
    schema: z.object({}).strict(),
  },
  9: {
  ...
  10: {
    ...
    description:
      "Phase 10: Run 10 meta-check questions, 6 final test questions, and adversarial challenge.",
    schema: z.object({}).strict(),
  },
} as const;

function proposalsToResult(
  proposals: EvidenceProposal[],
): PhaseToolResult["proposals"] {
...
function entitiesFromProposals(
  proposals: EvidenceProposal[],
): PhaseToolResult["entities_created"] {
...
export function registerPhaseTools(
  ...
  server.registerTool({
    ...
    async execute(input) {
      try {
        const currentAnalysis = context.getPipelineState().analysis_state;
        if (
          !currentAnalysis ||
          currentAnalysis.event_description !== input.situation_description
        ) {
        ...
        const phaseResult = context.getPipelineState().phase_results[1] as
          | { proposals: EvidenceProposal[] }
          | undefined;
        return successResult({
          ...
          nextStep:
            "Proceed with run_phase_2_players to identify strategic actors.",
        });
      } catch (error) {
        return failureResult(
          1,
          phaseDefinitions[1].phase_name,
  ...
  server.registerTool({
    ...
    async execute(input) {
      try {
        ...
        const phaseResult = context.getPipelineState().phase_results[2] as
          | { proposals: EvidenceProposal[] }
          | undefined;
        return successResult({
          phase: 2,
          ...
          proposals: phaseResult?.proposals ?? [],
          nextStep:
            "Proceed with run_phase_3_baseline after reviewing player proposals.",
        });
      } catch (error) {
        return failureResult(
          2,
          phaseDefinitions[2].phase_name,
  ...
  server.registerTool({
    ...
    async execute() {
      try {
        ...
        const phaseResult = context.getPipelineState().phase_results[3] as
          | { proposals: EvidenceProposal[] }
          | undefined;
        return successResult({
          phase: 3,
          ...
          proposals: phaseResult?.proposals ?? [],
          nextStep:
            "Proceed with run_phase_4_history after accepting the baseline model proposals.",
        });
      } catch (error) {
        return failureResult(
          3,
          phaseDefinitions[3].phase_name,
  ...
  server.registerTool({
    ...
    async execute() {
      try {
        ...
        const phaseResult = context.getPipelineState().phase_results[4] as
          | { proposals: EvidenceProposal[] }
          | undefined;
        return successResult({
          phase: 4,
          ...
          proposals: phaseResult?.proposals ?? [],
          nextStep:
            "Phase 4 complete. Revalidation or Phase 6 formalization would follow in later milestones.",
        });
      } catch (error) {
        return failureResult(
          4,
          phaseDefinitions[4].phase_name,
          error instanceof Error ? error.message : "Phase 4 failed.",
        );
      }
    },
  });
  ...
  server.registerTool({
    ...
    description:
      "Phase 5: review revalidation state, approve queued reruns, or dismiss pending triggers.",
    inputSchema: z
      .object({
    ...
    async execute(input): Promise<PhaseToolResult> {
      try {
        if (input.action === "status") {
          if (context.getPipelineState().analysis_state) {
          ...
          return failureResult(
            ...
            "event_id is required for approve and dismiss actions.",
          );
        } else if (input.action === "approve") {
          const outcome = await context.orchestrator.approveRevalidation(
            input.event_id,
          );
          if (!outcome) {
            return failureResult(
              5,
              "Recursive Revalidation",
              `No pending revalidation event found for ${input.event_id}.`,
            );
          }
        } else {
        ...
        const pipelineRuntime = context.getPipelineRuntimeState();
          ...
          revalidation: {
            ...
            active_rerun_cycle: pipelineRuntime.active_rerun_cycle
              ? {
                  ...
                  earliest_phase:
                    pipelineRuntime.active_rerun_cycle.earliest_phase,
        ...
        return failureResult(
          5,
          "Recursive Revalidation",
          error instanceof Error ? error.message : "Phase 5 failed.",
        );
      }
    },
  });
  ...
  server.registerTool({
    ...
    async execute(input): Promise<PhaseToolResult> {
      try {
        ...
        const phaseResult = context.getPipelineState().phase_results[6] as
          | FormalizationResult
          | undefined;
        ...
        return failureResult(
          6,
          phaseDefinitions[6].phase_name,
          error instanceof Error ? error.message : "Phase 6 failed.",
        );
      }
    },
  });
  ...
  server.registerTool({
    ...
    async execute(): Promise<PhaseToolResult> {
      try {
        ...
        const phaseResult = context.getPipelineState().phase_results[7] as
          | AssumptionExtractionResult
          | undefined;
        const criticalWarning = phaseResult?.sensitivity_summary
          .inference_only_critical
        ...
        return {
          ...
          proposals: proposalsToResult(phaseResult?.proposals ?? []),
          next_step:
            "Review Phase 7 assumption proposals, then continue with Phase 8.",
          warnings: [
            ...(phaseResult?.status.gaps ?? []),
        ...
        return failureResult(
          7,
          phaseDefinitions[7].phase_name,
          error instanceof Error ? error.message : "Phase 7 failed.",
        );
      }
    },
  });
  ...
  server.registerTool({
    ...
    async execute(): Promise<PhaseToolResult> {
      try {
        ...
        const phaseResult = context.getPipelineState().phase_results[8] as
          | EliminationResult
          | undefined;
        if (!phaseResult) {
          return failureResult(
            8,
            phaseDefinitions[8].phase_name,
            "Phase 8 produced no result.",
          );
        }
        return successResult({
          phase: 8,
          ...
          proposals: phaseResult.proposals,
          nextStep:
            "Review Phase 8 elimination proposals, then continue with Phase 9.",
        ...
        return failureResult(
          8,
          phaseDefinitions[8].phase_name,
          error instanceof Error ? error.message : "Phase 8 failed.",
        );
      }
    },
  });
  ...
  server.registerTool({
    ...
    async execute(): Promise<PhaseToolResult> {
      try {
        ...
        const phaseResult = context.getPipelineState().phase_results[9] as
          | ScenarioGenerationResult
          | undefined;
        if (!phaseResult) {
          return failureResult(
            9,
            phaseDefinitions[9].phase_name,
            "Phase 9 produced no result.",
          );
        }
        return successResult({
          phase: 9,
          ...
          proposals: phaseResult.proposals,
          nextStep:
            "Review Phase 9 scenario proposals, then continue with Phase 10.",
          warnings: phaseResult.status.gaps ?? [],
        });
      } catch (error) {
        return failureResult(
          9,
          phaseDefinitions[9].phase_name,
          error instanceof Error ? error.message : "Phase 9 failed.",
        );
      }
    },
  });
  ...
  server.registerTool({
    ...
    async execute(): Promise<PhaseToolResult> {
      try {
        ...
        const phaseResult = context.getPipelineState().phase_results[10] as
          | MetaCheckResult
          | undefined;
        if (!phaseResult) {
          return failureResult(
            10,
            phaseDefinitions[10].phase_name,
            "Phase 10 produced no result.",
          );
        }
        const proposals: EvidenceProposal[] =
          "proposals" in phaseResult
        ...
        return successResult({
          ...
          warnings: [
            ...
            ...phaseResult.meta_check_answers
              .filter(
                (a) =>
                  a.concern_level === "significant" ||
                  a.concern_level === "critical",
              )
              .map(
                (a) =>
                  `Q${a.question_number} (${a.concern_level}): ${a.answer}`,
              ),
          ],
        });
      } catch (error) {
        return failureResult(
          10,
          phaseDefinitions[10].phase_name,
          error instanceof Error ? error.message : "Phase 10 failed.",
        );
      }
    },
  });
