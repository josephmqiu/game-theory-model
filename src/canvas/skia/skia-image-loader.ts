...
export class SkiaImageLoader {
  ...
  request(src: string) {
    ...
    this.status.set(src, { state: 'loading' })
    const pending = this.loadAsync(src)
  ...
  private async loadAsync(src: string) {
    try {
      ...
      this.status.set(src, { state: skImg ? 'loaded' : 'error' })
      this.onLoaded?.()
    } catch (e) {
      console.warn('Failed to load image:', src.slice(0, 80), e)
      this.cache.set(src, null)
      this.loading.delete(src)
      this.status.set(src, { state: 'error' })
      this.onLoaded?.()
    }
  }
