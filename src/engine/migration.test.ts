...
describe('migration engine', () => {
  ...
  it('reports unsupported future versions', async () => {
    const result = await migrateFile({ schema_version: 9 }, 9, getCurrentSchemaVersion())
  ...
  it('collects discarded data for lossy transforms', async () => {
    const baseFile = storeToAnalysisFile(createSampleCanonicalStore(), createSampleAnalysisMeta())
    const v1Schema = z.object({
      schema_version: z.literal(1),
      name: z.string(),
      metadata: z.object({
  ...
  it('supports an end-to-end additive test transform defined only in the test suite', async () => {
    ...
    expect((result.data.metadata as Record<string, unknown>).primary_event_dates).toEqual({
      start: '2026-03-01',
    })
  })
})
