...
describe('stale propagation and audit persistence', () => {
  ...
  it('propagates stale marks transitively and clears them by cause', () => {
    ...
    expect(markResult.store.sources.source_1.stale_markers?.some((marker) => marker.caused_by.id === 'source_1')).toBe(true)
    expect(markResult.store.observations.observation_1.stale_markers?.some((marker) => marker.caused_by.id === 'source_1')).toBe(true)
    expect(markResult.store.claims.claim_1.stale_markers?.some((marker) => marker.caused_by.id === 'source_1')).toBe(true)
    ...
    const clearResult = dispatch(withSecondCause.store, withSecondCause.event_log, {
      kind: 'clear_stale',
      payload: { id: 'source_1' },
    })
    ...
    expect(clearResult.store.claims.claim_1.stale_markers?.some((marker) => marker.caused_by.id === 'claim_1')).toBe(true)
