...
const s = (d: string, id: string): IconEntry => ({
  d,
  style: "stroke",
  iconId: id,
});
const f = (d: string, id: string): IconEntry => ({
  d,
  style: "fill",
  iconId: id,
});
...
const ICON_PATH_MAP: Record<string, IconEntry> = {
  ...
  timer: s(
    "M10 2h4M12 6v4l2 2M21 12a9 9 0 11-18 0 9 9 0 0118 0",
    "lucide:timer",
  ),
  mappin: _MAPPIN,
  ...
  info: s(
    "M12 22a10 10 0 100-20 10 10 0 000 20zM12 8v4M12 16h.01",
    "lucide:info",
  ),
  alert: _ALERT,
  help: _HELP,
...
(function initIconSets() {
  ...
  const lucideIcons = (
    lucideData as { icons: Record<string, { body: string }> }
  ).icons;
  ...
  const lucideAliases = (
    lucideData as { aliases?: Record<string, { parent: string }> }
  ).aliases;
  if (lucideAliases) {
    for (const [alias, meta] of Object.entries(lucideAliases)) {
      const parentEntry =
        ICON_PATH_MAP[meta.parent] ??
        ICON_PATH_MAP[meta.parent.replace(/-/g, "")];
  ...
  for (const [alias, target] of Object.entries(commonAliases)) {
    const targetEntry =
      ICON_PATH_MAP[target] ?? ICON_PATH_MAP[target.replace(/-/g, "")];
  ...
  const featherIcons = (
    featherData as { icons: Record<string, { body: string }> }
  ).icons;
...
function tryImmediateIconResolution(nodeId: string, iconName: string): void {
  ...
  fetch(`/api/ai/icon?name=${encodeURIComponent(iconName)}`, {
 ...
 */
export function tryAsyncIconFontResolution(
  nodeId: string,
  iconName: string,
): void {
  ...
  fetch(`/api/ai/icon?name=${encodeURIComponent(normalized)}`, {
    signal: controller.signal,
  })
    .then((res) => (res.ok ? res.json() : null))
    .then((data) => {
...
export function applyIconPathResolution(node: PenNode): void {
  ...
  const originalNormalized = (node.name ?? node.id ?? "")
    .toLowerCase()
    .replace(/[-_\s]+/g, "");
  ...
  if (!match) {
    ...
    if (isIconLikeName(node.name ?? "", queueName)) {
      const fallback =
        ICON_PATH_MAP["circle"] ?? ICON_PATH_MAP["feather:circle"];
      if (fallback) {
        node.d = fallback.d;
...
export function applyNoEmojiIconHeuristic(node: PenNode): void {
  ...
  const iconFill =
    extractPrimaryColor("fill" in node ? node.fill : undefined) ?? "#64748B";
  ...
  const results = await Promise.allSettled(
    entries.map(async ({ nodeId, iconName }) => {
      const res = await fetch(
        `/api/ai/icon?name=${encodeURIComponent(iconName)}`,
      );
...
export const AVAILABLE_FEATHER_ICONS: readonly string[] =
  AVAILABLE_LUCIDE_ICONS;

// ---------------------------------------------------------------------------
// Built-in icon collection export — powers the icon picker collection
// ---------------------------------------------------------------------------
