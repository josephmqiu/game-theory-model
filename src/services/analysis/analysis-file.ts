...
import type { AnalysisFileV2, EntityAnalysis } from "@/types/entity";
import {
  normalizeAnalysis,
...
import { validateAnalysis as validateEntityAnalysis } from "@/services/entity/entity-validation";
...
function assertAnalysisWorkflowShape(
  rawWorkflow: unknown,
): asserts rawWorkflow is AnalysisWorkflowState {
  ...
  if (!isGuidedWorkflowStage(rawWorkflow.currentStage)) {
    throw new AnalysisFileError(
      "workflow.currentStage must be a valid guided workflow stage.",
    );
  }
}
...
function assertAnalysisShape(
  ...
  if (
    player1Raw.strategies.length === 0 ||
    player2Raw.strategies.length === 0
  ) {
    throw new AnalysisFileError(
      "Analysis must include at least one strategy per player.",
    );
  }
  ...
  for (const [index, strategy] of player1Raw.strategies.entries()) {
    if (!isRecord(strategy)) {
      throw new AnalysisFileError(
        `analysis.players[0].strategies[${index}] must be an object.`,
      );
    }
    assertAnalysisString(
      strategy.id,
      `analysis.players[0].strategies[${index}].id`,
    );
    assertAnalysisString(
      strategy.name,
      `analysis.players[0].strategies[${index}].name`,
    );
    if (player1StrategyIds.has(strategy.id)) {
      throw new AnalysisFileError(
        "analysis.players[0].strategies contains duplicate ids.",
      );
    }
  ...
  for (const [index, strategy] of player2Raw.strategies.entries()) {
    if (!isRecord(strategy)) {
      throw new AnalysisFileError(
        `analysis.players[1].strategies[${index}] must be an object.`,
      );
    }
    assertAnalysisString(
      strategy.id,
      `analysis.players[1].strategies[${index}].id`,
    );
    assertAnalysisString(
      strategy.name,
      `analysis.players[1].strategies[${index}].name`,
    );
    if (player2StrategyIds.has(strategy.id)) {
      throw new AnalysisFileError(
        "analysis.players[1].strategies contains duplicate ids.",
      );
    }
  ...
  if (rawAnalysis.profiles.length !== expectedProfileKeys.size) {
    throw new AnalysisFileError(
      "Analysis profiles do not match the strategy matrix size.",
    );
  }
  ...
  for (const [index, profile] of rawAnalysis.profiles.entries()) {
    if (!isRecord(profile)) {
      throw new AnalysisFileError(
        `analysis.profiles[${index}] must be an object.`,
      );
    }
    ...
    if (profile.payoffs.length !== 2) {
      throw new AnalysisFileError(
        `analysis.profiles[${index}].payoffs must contain two values.`,
      );
    }

    if (
      !isNumberOrNull(profile.payoffs[0]) ||
      !isNumberOrNull(profile.payoffs[1])
    ) {
      throw new AnalysisFileError(
        `analysis.profiles[${index}].payoffs must contain numbers or null.`,
      );
    }
    ...
    if (!expectedProfileKeys.has(profileKey)) {
      throw new AnalysisFileError(
        `analysis.profiles[${index}] references an unknown strategy combination.`,
      );
    }

    if (actualProfileKeys.has(profileKey)) {
      throw new AnalysisFileError(
        "Analysis profiles contain duplicate strategy combinations.",
      );
    }
  ...
  for (const expectedProfileKey of expectedProfileKeys) {
    if (!actualProfileKeys.has(expectedProfileKey)) {
      throw new AnalysisFileError(
        "Analysis profiles are missing one or more strategy combinations.",
      );
    }
  }
}
...
// ── v2 entity graph file format ──

const ANALYSIS_FILE_V2_VERSION = 2 as const;

function assertEntityAnalysisShape(
  rawAnalysis: unknown,
): asserts rawAnalysis is EntityAnalysis {
  if (!isRecord(rawAnalysis)) {
    throw new AnalysisFileError("Analysis payload must be an object.");
  }
  ...
  assertAnalysisString(rawAnalysis.id, "analysis.id");
  assertAnalysisString(rawAnalysis.name, "analysis.name");
  assertAnalysisString(rawAnalysis.topic, "analysis.topic");
  assertAnalysisArray(rawAnalysis.entities, "analysis.entities");
  assertAnalysisArray(rawAnalysis.relationships, "analysis.relationships");
  assertAnalysisArray(rawAnalysis.phases, "analysis.phases");
}

export function serializeEntityAnalysisFile(analysis: EntityAnalysis): string {
  const file: AnalysisFileV2 = {
    type: ANALYSIS_FILE_TYPE,
    version: ANALYSIS_FILE_V2_VERSION,
    analysis,
  };
  return JSON.stringify(file, null, 2);
}

export function parseEntityAnalysisFileText(text: string): EntityAnalysis {
  let raw: unknown;
  ...
  if (raw.version !== ANALYSIS_FILE_V2_VERSION) {
    throw new AnalysisFileError(
      `Unsupported analysis file version: ${String(raw.version)}.`,
    );
  }

  assertEntityAnalysisShape(raw.analysis);

  const validation = validateEntityAnalysis(raw.analysis);
...
export function createDefaultEntityAnalysisFileName(
