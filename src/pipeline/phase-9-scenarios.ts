...
function getEliminationResult(phaseResults?: Record<number, unknown>): EliminationResult | null {
  const result = phaseResults?.[8]
  return result && typeof result === 'object' && 'phase' in result && result.phase === 8
...
function findAssumptionRefsForFormalization(
  ...
  const relevantIds = new Set([...formalizationAssumptionIds, ...gameAssumptionIds])

  if (!phase7 || relevantIds.size === 0) {
    return [...relevantIds].map((id) => ({
      assumption_ref: asEntityRef('assumption', id),
      sensitivity: (canonical.assumptions[id]?.sensitivity ?? 'medium') as ScenarioAssumptionRef['sensitivity'],
    }))
  }
...
function deriveInvalidationConditions(
  ...
  return criticalRefs.map((ref) => {
    ...
    const phase7Assumption = phase7?.assumptions.find((a) => a.temp_id === assumptionId)
    if (phase7Assumption) {
...
function findCrossGameInteractions(
  ...
  return Object.values(canonical.cross_game_links)
    .filter((link) => link.source_game_id === gameId || link.target_game_id === gameId)
...
function estimateProbability(
  ...
  const baseConfidence = analysis
    ? analysis.readiness.completeness_score * analysis.readiness.confidence_floor
    : 0.3

  const adjustedForCount = gameCount > 1
    ? baseConfidence / gameCount
    : baseConfidence
...
function buildScenarios(
  ...
  for (const game of gamesWithFormalizations) {
    ...
    const fullText = [
      summary,
      ...causalChain.map((step) => `${step.phase}: ${step.event} -> ${step.leads_to}`),
    ].join('\n')
    ...
    scenarios.push({
      ...
      forecast_basis_explanation: forecastBasis === 'equilibrium'
...
function buildCentralThesis(
  ...
  if (scenarios.length === 0) {
    return {
      ...
      falsification_condition: 'Any structured scenario emerges from the model.',
      evidence_refs: [],
      assumption_refs: [],
      scenario_refs: [],
  ...
  const falsificationCondition = topScenario.invalidation_conditions[0]
    ?? `The scenario "${topScenario.name}" is contradicted by observed events.`

  const evidenceRefs = topScenario.model_basis
  const assumptionRefs = topScenario.key_assumptions.map((ref) => ref.assumption_ref)
  const scenarioRefs = scenarios.map((s) => s.temp_id)

  const forecastBasis: ForecastBasis = scenarios.every((s) => s.forecast_basis === 'equilibrium')
    ? 'equilibrium'
    ...
    const commands: Command[] = [
      {
        ...
        payload: {
          name: scenario.name,
          formalization_id: scenario.model_basis.find(
            (ref) => ref.type === 'formalization',
          )?.id ?? '',
...
function buildTailRiskProposals(
  ...
  return tailRisks.map((risk) => {
    const relatedScenarioRefs: EntityRef[] = scenarios.map(
      (s) => asEntityRef('scenario', s.temp_id),
    )
...
function buildThesisProposal(
  ...
  const scenarioEntityRefs: EntityRef[] = thesis.scenario_refs.map(
    (tempId) => asEntityRef('scenario', tempId),
  )
...
export function runPhase9Scenarios(
  ...
  if (!phase6) {
    const emptyThesis: ProposedCentralThesis = {
      ...
      falsification_condition: 'Phase 6 formalization results become available.',
      evidence_refs: [],
      assumption_refs: [],
      scenario_refs: [],
      forecast_basis: 'discretionary',
    ...
    const status: PhaseResult = {
      ...
      retriable: true,
      gaps: ['Phase 6 formalization results are not available; scenario generation requires equilibrium analysis.'],
    }
  ...
  const tailRiskProposals = buildTailRiskProposals(tailRisks, scenarios, context)
  const thesisProposal = buildThesisProposal(centralThesis, context)

  const proposals = [...scenarioProposals, ...tailRiskProposals, thesisProposal]

  const gaps: string[] = []
  if (scenarios.length === 0) {
    gaps.push('No games with formalizations were found; no scenarios could be generated.')
  }
  if (!phase7) {
    gaps.push('Phase 7 assumption results are not available; tail risk identification was skipped.')
  }
  if (!_phase8) {
    gaps.push('Phase 8 elimination results are not available; eliminated outcomes were not factored in.')
  }
