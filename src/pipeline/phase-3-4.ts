...
function inferCanonicalGameType(description: string): ProposedBaselineGame['canonical_type'] {
  if (descriptionContains(description, 'sanction', 'threat', 'brink', 'strike', 'escalat')) return 'chicken_brinkmanship'
  if (descriptionContains(description, 'entry', 'incumbent', 'market')) return 'entry_deterrence'
  if (descriptionContains(description, 'price', 'collusion', 'firm')) return 'prisoners_dilemma'
  if (descriptionContains(description, 'signal', 'credib', 'audience')) return 'signaling'
  return 'bargaining'
}

function buildStrategyTable(playerIds: string[], description: string): ProposedStrategyTable {
  ...
  return {
    ...
    strategies: playerIds.map((playerId, index) => ({
      player_id: playerId,
      strategies: (baseStrategies[index] ?? ['Advance', 'Delay']).map((label) => ({
        label,
...
function buildBaselineGame(context: Phase34RunnerContext): ProposedBaselineGame {
  const players = Object.keys(context.canonical.players).slice(0, 2)
  const canonical_type = inferCanonicalGameType(context.analysisState.event_description)
  return {
    ...
    move_order: descriptionContains(context.analysisState.event_description, 'sequence', 'deadline', 'ultimatum') ? 'sequential' : 'simultaneous',
    time_structure: {
    ...
    deterrence_vs_compellence: descriptionContains(context.analysisState.event_description, 'force', 'threat', 'deterr')
      ? 'deterrence'
      : descriptionContains(context.analysisState.event_description, 'compel', 'pressure')
        ? 'compellence'
        : 'both',
    institutional_constraints: [
      {
        category: 'regulatory',
        description: 'Existing institutional constraints limit how quickly actors can change course.',
        constraining_effect: 'Raises switching costs and slows strategic pivots.',
        evidence_refs: [],
      },
    ],
    adjacent_game_test: 'uncertain',
    evidence_refs: [],
    confidence: createConfidenceEstimate('Baseline game fit from Phase 1 and Phase 2 structure.', [], 0.72),
    rationale: 'Chosen as the smallest canonical game that captures the main strategic tension.',
  }
}
...
function buildBaselineProposals(
  ...
  const payoff_cells = strategyTable.strategies[0]?.strategies.flatMap((rowStrategy) =>
    ...
    commands: [
      {
        ...
        payload: {
          ...
          semantic_labels: [baselineGame.canonical_type === 'chicken_brinkmanship' ? 'chicken' : 'bargaining'],
...
export function runPhase3Baseline(context: Phase34RunnerContext): BaselineModelResult {
  ...
  if (playerIds.length < 2) {
    return {
      ...
      model_gaps: ['Need accepted player proposals before creating a baseline game.'],
      proposals: [],
    }
  }

  const baselineGame = buildBaselineGame(context)
  const strategyTable = buildStrategyTable(baselineGame.players, context.analysisState.event_description)
  const proposals = buildBaselineProposals(baselineGame, strategyTable, context)

  const ladderProposal = proposals.find((proposal) => proposal.proposal_type === 'escalation')
  const tableProposal = proposals.find((proposal) => proposal.proposal_type === 'constraint_table')
...
export function runPhase4History(context: Phase34RunnerContext): HistoricalGameResult {
  const playerIds = Object.keys(context.canonical.players)
  ...
  if (playerIds.length < 2 || games.length === 0) {
    return {
      ...
      status: {
        ...
        execution_id: context.phaseExecution.id,
        retriable: true,
        error: 'Phase 4 requires accepted players and a baseline game from Phase 3.',
      },
      repeated_game_map: [],
      patterns_found: [],
      trust_assessment: [],
  ...
  const patterns_found: ProposedRepeatedGamePattern[] = [
    {
      game_id: games[0]!.id,
      pattern_type: descriptionContains(context.analysisState.event_description, 'deception', 'bluff')
        ? 'dual_track_deception'
        : 'tit_for_tat',
      description: 'The interaction history shows reciprocal responses to prior moves.',
      instances: repeated_game_map.map((entry) => ({
        date: entry.date,
        description: entry.description,
        evidence_refs: [],
      })),
      impact_on_trust: 'Trust decays when punitive cycles dominate the interaction history.',
      impact_on_model: 'The game behaves more like a repeated interaction than a one-shot exchange.',
    },
  ]
  ...
  const trust_assessment: ProposedTrustAssessment[] = [
    {
      ...
      level: descriptionContains(context.analysisState.event_description, 'alliance', 'cooperate') ? 'moderate' : 'low',
      posterior_belief: createEstimate(0.42, 'Posterior trust is constrained by recent punitive interactions.'),
      evidence_refs: [],
      interaction_history_summary: 'Past cooperation has repeatedly broken down under pressure.',
      driving_patterns: [],
      implications: 'Low trust increases the value of enforceable commitments and visible costly signals.',
    },
  ]
  ...
  const dynamic_inconsistency_risks: ProposedDynamicInconsistencyRisk[] = [
    {
      ...
      commitment_description: 'Leadership incentives may drift after the immediate crisis window.',
      risk_type: descriptionContains(context.analysisState.event_description, 'election', 'vote') ? 'electoral_cycle' : 'leadership_transition',
      durability: descriptionContains(context.analysisState.event_description, 'volatile', 'transition') ? 'fragile' : 'moderate',
      evidence_refs: [],
      affected_games: [asEntityRef('game', games[0]!.id)],
      mitigation: 'Tie commitments to observable milestones or third-party verification.',
    },
  ]
  ...
  return {
    ...
    baseline_recheck: {
      ...
      hidden_player_found: false,
      hidden_commitment_problem: dynamic_inconsistency_risks[0]!.durability === 'fragile',
      hidden_type_uncertainty: trust_assessment[0]!.level === 'low',
      cooperative_equilibria_eliminated: false,
      objective_function_changed: dynamic_inconsistency_risks[0]!.durability === 'fragile',
      deterrence_compellence_reframed: false,
      revalidation_needed: revalidationNeeded,
      revalidation_triggers: revalidationNeeded
        ? [dynamic_inconsistency_risks[0]!.durability === 'fragile' ? 'objective_function_changed' : 'repeated_dominates_oneshot']
        : [],
    },
    proposals,
  }
