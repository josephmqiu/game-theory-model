...
import type { AddEntityCommand, CrudCommand, DeleteEntityCommand, EntityFor, UpdateEntityCommand } from '../../engine/commands'
...
function getExistingEntity(
  ...
  return canonical[storeKey][entity_id] as Record<string, unknown> | undefined ?? null
}
...
function buildCommandForProposal(input: {
  ...
  if (input.action === 'update') {
    const existing = getExistingEntity(input.canonical, input.entity_type, input.entity_id)
    if (!existing) {
      throw new Error(`Cannot update missing ${input.entity_type} ${input.entity_id}.`)
    }
    ...
    const { id, ...payload } = parsed
    return {
      command: createUpdateCommand(input.entity_type, id, payload) as CrudCommand,
      preview: payload,
    }
  }

  return {
    command: createDeleteCommand(input.entity_type, input.entity_id) as CrudCommand,
