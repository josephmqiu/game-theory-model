...
function getPhase2Check(result: PlayerIdentificationResult): RevalidationCheck {
  ...
  if (internalPlayers.length === 0) {
    return createCheck(
      ...
      [],
      "none",
      "Phase 2 introduced no new independent agency triggers.",
    );
  }
...
function getPhase3Check(result: BaselineModelResult): RevalidationCheck {
  ...
  if (result.proposed_games.length > 1) {
    ...
    affectedEntities.push(
      ...result.proposed_games.map((game) =>
        createEntityRef("game", game.temp_id),
      ),
    );
  ...
  if (result.escalation_ladder) {
    ...
    affectedEntities.push(
      createEntityRef("game", result.escalation_ladder.game_id),
    );
    notes.push("Escalation ladder changed the baseline framing.");
  }

  const constrainedGames = result.proposed_games.filter(
    (game) => game.institutional_constraints.length > 0,
  );
  if (constrainedGames.length > 0) {
    triggers.push("institutional_constraint_changed");
    affectedPhases.push(3);
    affectedEntities.push(
      ...constrainedGames.map((game) => createEntityRef("game", game.temp_id)),
    );
    notes.push(
      "Institutional constraints materially shape the strategy space.",
    );
  }

  if (
    result.model_gaps.some((gap) =>
      /does not yet explain|remain approximate/i.test(gap),
    )
  ) {
  ...
  if (triggers.length === 0) {
    return createCheck(
      ...
      [],
      "none",
      "Phase 3 introduced no revalidation triggers.",
    );
  }

  const recommendation: RevalidationCheck["recommendation"] = triggers.includes(
    "new_game_identified",
  )
    ? "revalidate"
    : "monitor";
...
function getPhase4Check(result: HistoricalGameResult): RevalidationCheck {
  if (!result.baseline_recheck.revalidation_needed) {
    return createCheck(
      ...
      [],
      "none",
      "Phase 4 historical review converged with the current baseline.",
    );
  }

  const gameRefs = result.dynamic_inconsistency_risks.flatMap(
    (risk) => risk.affected_games,
  );
...
function getPhase6Check(result: FormalizationResult): RevalidationCheck {
  ...
  if (triggers.length === 0) {
    return createCheck(
      ...
      [],
      "none",
      "Phase 6 introduced no revalidation-worthy formalization shifts.",
    );
  }

  const affectedPhases = triggers.flatMap(
    (t) => PHASE6_TRIGGER_TARGETS[t] ?? [6],
  );
...
function getPhase7Check(
  result: AssumptionExtractionResult,
  canonical: CanonicalStore,
  getDerivedState: RevalidationEngineDependencies["getDerivedState"],
): RevalidationCheck {
  ...
  const triggered = result.assumptions.filter((assumption) => {
    ...
    const invalidated = Boolean(
      (canonicalAssumption.contradicted_by ?? []).length ||
      (canonicalAssumption.stale_markers ?? []).length,
    );
    if (!invalidated) {
      return false;
    }
    ...
    return affectedFormalizations.some((formalizationId) =>
      Object.values(
        derived.sensitivityByFormalizationAndSolver[formalizationId] ?? {},
      ).some((analysis) =>
        analysis?.assumption_sensitivities.some(
          (entry) =>
            entry.assumption_id === assumption.temp_id &&
  ...
  if (triggered.length === 0) {
    return createCheck(
      ...
      [],
      "none",
      "Phase 7 introduced no invalidated critical empirical assumptions.",
    );
  }
...
function getPhase10Check(result: MetaCheckResult): RevalidationCheck {
  const significantConcerns = result.meta_check_answers.filter(
    (answer) =>
      answer.concern_level === "significant" ||
      answer.concern_level === "critical",
  );
  if (significantConcerns.length === 0) {
    return createCheck(
      ...
      [],
      "none",
      "Phase 10 meta-check found no significant concerns.",
    );
  }
  ...
  if (triggers.length === 0) {
    return createCheck(
      ...
      [],
      "monitor",
      `Phase 10 meta-check flagged ${significantConcerns.length} concern(s) without specific revision triggers.`,
    );
  }

  const affectedPhases = triggers.flatMap(
    (t) => PHASE10_TRIGGER_TARGETS[t] ?? [],
  );
...
export function createRevalidationEngine(
  ...
  return {
    checkTriggers(phaseResult, phase) {
      ...
      if (phase === 7) {
        return getPhase7Check(
          phaseResult as AssumptionExtractionResult,
          deps.getCanonical(),
          deps.getDerivedState,
        );
      }
      if (phase === 10) {
      ...
      return createCheck(
        ...
        [],
        "none",
        `Phase ${phase} has no implemented trigger checks yet.`,
      );
    },
    ...
    async executeRevalidation(event) {
      ...
      const nextPassNumber =
        (analysisState?.pass_number ?? event.pass_number) + 1;
      ...
      return {
        ...
        entities_marked_stale:
          pendingApproval?.affected_entities ?? event.entity_refs,
        entities_updated: [],
