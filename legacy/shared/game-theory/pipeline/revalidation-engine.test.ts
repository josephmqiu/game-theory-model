...
describe("revalidation engine", () => {
  ...
  it("detects repeated-game revalidation from phase 4 baseline recheck output", () => {
    ...
    expect(check.affected_entities).toContainEqual({
      type: "game",
      id: "game_1",
    });
  });
  ...
  it("triggers revalidation from phase 10 when meta-check finds critical concerns", () => {
    ...
    const result: MetaCheckResult = {
      ...
      meta_check_answers: Array.from({ length: 10 }, (_, i) => ({
        ...
        answer:
          i === 2 ? "The baseline model misses a key player." : "No concerns.",
        concern_level: (i === 2 ? "critical" : "none") as "critical" | "none",
        revision_triggered: (i === 2 ? "model_cannot_explain_fact" : null) as
          | "model_cannot_explain_fact"
          | null,
        evidence_refs: [],
      })),
      ...
      adversarial_result: {
        challenges: [],
        overall_assessment: "defensible" as const,
      },
  ...
  it("returns none when phase 10 meta-check has no significant concerns", () => {
    ...
    const result: MetaCheckResult = {
      ...
      adversarial_result: {
        challenges: [],
        overall_assessment: "robust" as const,
      },
      revisions_triggered: [],
  ...
  it("returns monitor when phase 10 has significant concerns without revision triggers", () => {
    ...
    const result: MetaCheckResult = {
      ...
      meta_check_answers: Array.from({ length: 10 }, (_, i) => ({
        ...
        concern_level: (i === 4 ? "significant" : "none") as
          | "significant"
          | "none",
        revision_triggered: null,
        evidence_refs: [],
