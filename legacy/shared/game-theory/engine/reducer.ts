...
export class CommandError extends Error {
  ...
  constructor(
    message: string,
    reason: "validation_failed" | "error" = "validation_failed",
  ) {
    super(message);
...
function appendStaleMarker(
  ...
  if (!entity) {
    throw new CommandError(
      `Cannot mark stale for missing entity ${target.id}.`,
    );
  }
  ...
  const duplicate = staleMarkers.some(
    (candidate) => refKey(candidate.caused_by) === refKey(target),
  );
  if (!duplicate) {
...
function removeStaleMarker(
  ...
  if (!entity) {
    throw new CommandError(
      `Cannot clear stale for missing entity ${target.id}.`,
    );
  }
...
function reduceCrudCommand(
  ...
  if (descriptor.operation === "update") {
    ...
    if (!existing) {
      throw new CommandError(
        `Entity ${updateCommand.payload.id} does not exist.`,
      );
    }
  ...
  if (!(deleteCommand.payload.id in record)) {
    throw new CommandError(
      `Entity ${deleteCommand.payload.id} does not exist.`,
    );
  }
...
function reduceBatch(
  store: CanonicalStore,
  command: Extract<Command, { kind: "batch" }>,
): CanonicalStore {
  return command.commands.reduce(
    (current, nested) => reduceStore(current, nested),
    store,
  );
}
...
export function reduceStore(
  ...
  switch (command.kind) {
    case "update_payoff": {
      ...
      if (!node) {
        throw new CommandError(
          `Node ${command.payload.node_id} does not exist.`,
        );
      }
    ...
    case "update_normal_form_payoff": {
      const nextStore = cloneStore(store);
      const formalization =
        nextStore.formalizations[command.payload.formalization_id];
      if (!formalization) {
        throw new CommandError(
      ...
      if (!cell) {
        const { row_strategy: rowStrategy, col_strategy: colStrategy } =
          command.payload;
        ...
        if (playerIds.length < 2) {
          throw new CommandError(
            "Normal-form formalization must have two players before editing payoffs.",
          );
        }
    ...
    case "attach_player_to_game": {
      ...
      if (!game) {
        throw new CommandError(
          `Game ${command.payload.game_id} does not exist.`,
        );
      }
      if (!(command.payload.player_id in nextStore.players)) {
        throw new CommandError(
          `Player ${command.payload.player_id} does not exist.`,
        );
      }
    ...
    case "attach_formalization_to_game": {
      ...
      if (!game.formalizations.includes(command.payload.formalization_id)) {
        game.formalizations = [
          ...game.formalizations,
          command.payload.formalization_id,
        ];
      }
      return nextStore;
    }
    ...
    case "remove_player_from_game": {
      ...
      game.players = game.players.filter(
        (playerId) => playerId !== command.payload.player_id,
      );
      return nextStore;
    }
    ...
    case "promote_play_result":
      throw new CommandError(
        `Command kind ${command.kind} is not implemented yet.`,
      );
  }

  throw new CommandError(
    `Unsupported command kind ${JSON.stringify(command)}.`,
  );
}

export function reduce(store: CanonicalStore, command: Command): ReduceResult {
