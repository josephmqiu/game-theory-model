...
import type { CanonicalStore, Formalization } from "../types/index";
...
export function arrayToMap<T extends { id: string }>(
  ...
  for (const item of items) {
    if (item.id in record) {
      throw new Error(
        `Duplicate entity id "${item.id}" encountered during array-to-map conversion.`,
      );
    }
...
export function mapToArray<T extends { id: string }>(
...
function withoutReadinessCache(
  formalization: Formalization,
): PersistedFormalization {
...
export function storeToAnalysisFile(
  ...
  return {
    ...
    cross_game_constraint_tables: mapToArray(
      store.cross_game_constraint_tables,
    ),
