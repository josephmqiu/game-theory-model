...
import type { CanonicalStore, EntityRef, EstimateValue, Formalization } from '../types'
import type {
...
type OwnerAttachment =
  ...
  | { kind: 'normal_form_estimate'; formalization_id: string; cell_index: number; player_id: string }
...
function getHistoricalResult(phaseResults?: Record<number, unknown>): HistoricalGameResult | null {
  const result = phaseResults?.[4]
  return result && typeof result === 'object' && 'phase' in result && result.phase === 4
...
function normalizeStatement(statement: string): string {
  return statement.trim().toLowerCase().replace(/[^a-z0-9]+/g, ' ').replace(/\s+/g, ' ')
}
...
function createStableTempId(statement: string, ownerKey?: string): string {
  const normalized = normalizeStatement(statement).replace(/\s+/g, '_').slice(0, 48) || 'assumption'
...
function pickTopToken(statement: string): string {
  const stopwords = new Set([
    'the', 'and', 'that', 'this', 'with', 'from', 'into', 'over', 'under', 'same', 'remain',
    'players', 'player', 'current', 'model', 'game', 'will', 'would', 'could', 'their',
  ])
...
function isClaimBackedByObservation(claimId: string, canonical: CanonicalStore): boolean {
...
function confidenceForCandidate(
  ...
  const baseline = evidenceQuality === 'direct_evidence'
    ? 0.82
...
function buildOwnerIndexes(canonical: CanonicalStore): Map<string, OwnerAttachment[]> {
  ...
  for (const node of Object.values(canonical.nodes)) {
    ...
    Object.entries(node.terminal_payoffs ?? {}).forEach(([playerId, estimate]) => {
      for (const assumptionId of estimate.assumptions ?? []) {
        pushOwner(assumptionId, {
...
function gatherAffectedConclusions(
  ...
  for (const owner of owners) {
    ...
    } else if (owner.kind === 'formalization' || owner.kind === 'normal_form_estimate') {
      refs.push(asEntityRef('formalization', owner.kind === 'formalization' ? owner.id : owner.formalization_id))
    } else if (owner.kind === 'game_node' || owner.kind === 'terminal_estimate') {
    ...
    } else if (owner.kind === 'game_edge' || owner.kind === 'edge_payoff_delta') {
...
function collectCanonicalCandidates(
  ...
  return Object.values(canonical.assumptions)
    .map((assumption) => {
      ...
      return {
        ...
        extra_evidence_refs: [],
        affected_conclusions: gatherAffectedConclusions(canonical, assumption.id, owners),
        game_theoretic_vs_empirical: assumption.game_theoretic_vs_empirical ?? 'empirical',
...
function collectHistoricalCandidates(
  ...
  const repeatedGames = uniqueStrings(historical.patterns_found.map((pattern) => pattern.game_id))
  for (const gameId of repeatedGames) {
    const game = canonical.games[gameId]
    candidates.push({
      existing_id: null,
      temp_id: createStableTempId(`The interaction in ${game?.name ?? gameId} remains strategically repeated rather than resetting to a one-shot encounter.`, gameId),
  ...
  if (historical.baseline_recheck.hidden_type_uncertainty) {
    ...
    if (gameId) {
        candidates.push({
          existing_id: null,
          temp_id: createStableTempId('Relevant player types, resolve, or private constraints remain partially hidden to opponents.', gameId),
          statement: 'Relevant player types, resolve, or private constraints remain partially hidden to opponents.',
        type: 'information',
        owners: [{ kind: 'game', id: gameId }],
        supported_by_ids: [],
        contradicted_by_ids: [],
  ...
  for (const assessment of historical.trust_assessment) {
    const targetLabel = canonical.players[assessment.target_player_id]?.name ?? assessment.target_player_id
    candidates.push({
      existing_id: null,
      temp_id: createStableTempId(`${targetLabel} will keep behaving in line with the current trust assessment rather than unexpectedly restoring cooperation.`, assessment.target_player_id),
  ...
  for (const risk of historical.dynamic_inconsistency_risks) {
    const playerLabel = canonical.players[risk.player_id]?.name ?? risk.player_id
    candidates.push({
      existing_id: null,
      temp_id: createStableTempId(`${playerLabel} can sustain the current commitment posture without a decisive internal reversal.`, risk.player_id),
      ...
      game_theoretic_vs_empirical: 'empirical',
      confidence_value: risk.durability === 'durable' ? 0.76 : risk.durability === 'moderate' ? 0.66 : 0.56,
    })
  }
...
function synthesizeFormalizationAssumption(
  ...
  const statement = kind === 'bargaining'
  ...
  return {
    ...
    statement,
    type: kind === 'signaling' || kind === 'bayesian' ? 'information' : 'structural',
    owners: formalization ? [{ kind: 'formalization', id: formalization.id }] : [],
    supported_by_ids: [],
    contradicted_by_ids: [],
    extra_evidence_refs: formalization ? [asEntityRef('formalization', formalization.id)] : [],
    affected_conclusions: formalization ? [asEntityRef('formalization', formalization.id)] : [],
    game_theoretic_vs_empirical: 'game_theoretic',
...
function collectFormalizationCandidates(
  ...
  for (const summary of phase6.formal_representations.summaries) {
    candidates.push(synthesizeFormalizationAssumption(canonical, summary.formalization_id, summary.kind))
  }
  ...
  for (const selection of phase6.equilibrium_selection.selections) {
    ...
    candidates.push({
      existing_id: null,
      temp_id: createStableTempId(`Players in ${game?.name ?? selection.formalization_id} will keep selecting among plausible equilibria using the current focal, institutional, or commitment cues.`, selection.formalization_id),
      ...
      supported_by_ids: [],
      contradicted_by_ids: [],
      extra_evidence_refs: [asEntityRef('formalization', selection.formalization_id)],
      affected_conclusions: [asEntityRef('formalization', selection.formalization_id)],
      game_theoretic_vs_empirical: 'game_theoretic',
      confidence_value: 0.7,
    })
  }
...
function mergeCandidates(candidates: CandidateAssumption[], canonical: CanonicalStore): CandidateAssumption[] {
  const canonicalByStatement = new Map<string, string>()
  for (const assumption of Object.values(canonical.assumptions)) {
    canonicalByStatement.set(normalizeStatement(assumption.statement), assumption.id)
  }
  ...
  for (const candidate of candidates) {
    ...
    const existingId = candidate.existing_id ?? canonicalByStatement.get(normalized) ?? null
    const key = existingId ? `existing:${existingId}` : `${normalized}:${JSON.stringify(candidate.owners[0] ?? null)}`
    ...
    merged.set(key, {
      ...
      supported_by_ids: uniqueStrings([...current.supported_by_ids, ...candidate.supported_by_ids]),
      contradicted_by_ids: uniqueStrings([...current.contradicted_by_ids, ...candidate.contradicted_by_ids]),
      extra_evidence_refs: uniqueRefs([...current.extra_evidence_refs, ...candidate.extra_evidence_refs]),
      affected_conclusions: uniqueRefs([...current.affected_conclusions, ...candidate.affected_conclusions]),
      confidence_value: Math.max(current.confidence_value, candidate.confidence_value),
    })
  }

  return [...merged.values()].sort((left, right) => left.statement.localeCompare(right.statement))
}

function collectAssumptionSensitivities(formalizationId: string): AssumptionSensitivity[] {
  const state = getDerivedStore().getState()
  const analyses = state.sensitivityByFormalizationAndSolver[formalizationId]
  if (!analyses) {
    return []
  }

  const entries = Object.values(analyses).flatMap((analysis) => analysis?.assumption_sensitivities ?? [])
  const seen = new Set<string>()
...
function determineSensitivity(
  ...
  const anchorFormalizationId = phase6?.formal_representations.summaries[0]?.formalization_id ?? null
  const selectedFormalizationIds = new Set(phase6?.equilibrium_selection.selections.map((entry) => entry.formalization_id) ?? [])
  const affectedFormalizationIds = new Set(
    candidate.affected_conclusions
      .filter((ref) => ref.type === 'formalization')
      .map((ref) => ref.id),
  )
  const sensitivityHits = [...affectedFormalizationIds].flatMap((formalizationId) =>
    collectAssumptionSensitivities(formalizationId),
  )

  const flipsResult = candidate.existing_id
    ? sensitivityHits.some((entry) => entry.assumption_id === candidate.existing_id && entry.impact === 'result_changes')
    : sensitivityHits.some((entry) => entry.impact === 'result_changes')

  const touchesAnchor = Boolean(
    (anchorFormalizationId && affectedFormalizationIds.has(anchorFormalizationId))
    || [...selectedFormalizationIds].some((formalizationId) => affectedFormalizationIds.has(formalizationId)),
  )
...
function ownerSimilarity(
  left: Pick<FinalizedAssumption, 'statement' | 'supported_by_ids' | 'affected_conclusions'>,
  right: Pick<FinalizedAssumption, 'statement' | 'supported_by_ids' | 'affected_conclusions'>,
): boolean {
  const leftConclusions = new Set(left.affected_conclusions.map((ref) => `${ref.type}:${ref.id}`))
  const rightConclusions = new Set(right.affected_conclusions.map((ref) => `${ref.type}:${ref.id}`))
  const sharedConclusion = [...leftConclusions].some((key) => rightConclusions.has(key))
  if (sharedConclusion) {
    return true
  }
...
function buildClusters(
  ...
  for (const assumption of assumptions) {
    parent.set(assumption.temp_id, assumption.correlated_cluster_id ?? assumption.temp_id)
  }

  for (let index = 0; index < assumptions.length; index += 1) {
    for (let nextIndex = index + 1; nextIndex < assumptions.length; nextIndex += 1) {
  ...
  const clusteredAssumptions = assumptions.map((assumption) => ({ ...assumption }))
  ...
  for (const group of grouped.values()) {
    ...
    const explicitId = group.map((entry) => entry.correlated_cluster_id).find((value) => value)
    const topToken = pickTopToken(group[0]!.statement)
    const clusterId = explicitId ?? `cluster_${topToken}`
    const affectedDomains = uniqueStrings(group.flatMap((entry) =>
  ...
  clusteredAssumptions.sort((left, right) => left.statement.localeCompare(right.statement))
...
function buildSensitivitySummary(
  ...
  return {
    critical_count: assumptions.filter((assumption) => assumption.sensitivity === 'critical').length,
    high_count: assumptions.filter((assumption) => assumption.sensitivity === 'high').length,
    medium_count: assumptions.filter((assumption) => assumption.sensitivity === 'medium').length,
    low_count: assumptions.filter((assumption) => assumption.sensitivity === 'low').length,
    inference_only_critical: assumptions.filter((assumption) =>
      assumption.sensitivity === 'critical' && assumption.evidence_quality !== 'direct_evidence',
    ).length,
    largest_cluster_size: clusters.reduce((largest, cluster) => Math.max(largest, cluster.assumption_ids.length), 0),
  }
}
...
function buildFinalizedAssumptions(
  ...
  return candidates.map((candidate) => {
    const evidenceQuality = inferEvidenceQuality(candidate.supported_by_ids, canonical)
    const sensitivity = determineSensitivity(candidate, phase6)
    return {
      ...
      correlated_cluster_id: candidate.existing_id
        ? canonical.assumptions[candidate.existing_id]?.correlated_cluster_id ?? null
        : null,
      evidence_quality: evidenceQuality,
      evidence_refs: uniqueRefs([
        ...candidate.extra_evidence_refs,
        ...candidate.supported_by_ids.map((id) =>
          id in canonical.claims ? asEntityRef('claim', id) : asEntityRef('inference', id),
        ),
      ]),
...
function attachAssumptionCommands(
  ...
  if (owner.kind === 'normal_form_estimate') {
    ...
    const payoff_cells = formalization.payoff_cells.map((existingCell, index) =>
...
function buildProposal(
  ...
  const supported_by = uniqueStrings(assumption.supported_by_ids.filter((id) =>
    id in context.canonical.claims || id in context.canonical.inferences,
  ))
  const confidenceValue = assumption.confidence.value ?? assumption.confidence.confidence ?? 0.6
  ...
  if (assumption.existing_id) {
    ...
    if ((existing.game_theoretic_vs_empirical ?? 'empirical') !== assumption.game_theoretic_vs_empirical) {
      payload.game_theoretic_vs_empirical = assumption.game_theoretic_vs_empirical
    }
    if ((existing.correlated_cluster_id ?? null) !== assumption.correlated_cluster_id) {
    ...
    const updateCommand: Command = {
      ...
      payload: payload as { id: string } & Partial<CanonicalStore['assumptions'][string]>,
    }
...
export function runPhase7Assumptions(
  ...
  const finalized = buildFinalizedAssumptions(mergedCandidates, context.canonical, phase6)
  ...
  if (clustered.assumptions.some((assumption) => assumption.sensitivity === 'critical' && assumption.evidence_quality !== 'direct_evidence')) {
    gaps.push('One or more critical assumptions rely on inference rather than direct evidence.')
  }
  ...
  return {
    ...
    sensitivity_summary: buildSensitivitySummary(clustered.assumptions, clustered.clusters),
    proposals,
  }
}
