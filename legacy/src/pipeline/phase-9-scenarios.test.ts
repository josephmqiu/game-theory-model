...
describe('Phase 9 scenario generation runner', () => {
  ...
  it('generates scenarios with narrative and probability from game formalizations', () => {
    ...
    for (const scenario of result.proposed_scenarios) {
      ...
      expect(['equilibrium', 'discretionary', 'mixed']).toContain(scenario.forecast_basis)
    }
  })
  ...
  it('produces a central thesis with falsification condition', () => {
    ...
    expect(result.central_thesis.falsification_condition.length).toBeGreaterThan(0)
  ...
  it('checks probability sum', () => {
    ...
    expect(result.probability_check.missing_probability).toBeGreaterThanOrEqual(0)
  ...
  it('produces add_scenario and add_central_thesis proposals', () => {
    ...
    const scenarioProposals = result.proposals.filter(
      (proposal) => proposal.commands.some((command) => command.kind === 'add_scenario'),
    )
    ...
    const thesisProposals = result.proposals.filter(
      (proposal) => proposal.commands.some((command) => command.kind === 'add_central_thesis'),
    )
