...
describe('M4 solvers', () => {
  ...
  it('eliminates dominated strategies across rounds', () => {
    ...
    const result = eliminateDominance(store.formalizations.formalization_1 as NormalFormModel, store)
  ...
  it('chains Bayesian posteriors across sequential observations', () => {
    ...
    const result = computeBayesianUpdate(store.formalizations.formalization_1 as BayesianGameModel, store)
  ...
  it('returns partial Bayesian results when one observation branch is invalid', () => {
    ...
    expect(result.posterior_beliefs.some((belief) => belief.evidence_used[0] === 'hawkish')).toBe(true)
  })
  ...
  it('renormalizes posteriors after clamping', () => {
    ...
    const result = computeBayesianUpdate(store.formalizations.formalization_1 as BayesianGameModel, store)
    const needleBeliefs = result.posterior_beliefs.filter((belief) => belief.evidence_used[0] === 'needle')
    const totalPosterior = needleBeliefs.reduce((sum, belief) => sum + belief.posterior, 0)
  ...
  it('skips mixed-strategy search outside 2x2 games with a warning', () => {
        ...
        formalization_1: {
          ...
          payoff_cells: [
            ...formalization.payoff_cells,
            {
              ...
              payoffs: { player_1: createEstimate(2), player_2: createEstimate(4) },
            },
            {
              strategy_profile: { player_1: 'Delay', player_2: 'Defect' },
              payoffs: { player_1: createEstimate(0.5), player_2: createEstimate(1.5) },
            },
          ],
        },
      },
    ...
    const result = solveNash(store.formalizations.formalization_1 as NormalFormModel, store)

    expect(result.status).toBe('success')
    expect(result.warnings.join(' ')).toContain('Mixed-strategy search is limited to 2x2 games')
    expect(result.equilibria.every((equilibrium) => equilibrium.type === 'pure')).toBe(true)
  })
  ...
  it('marks only linked assumptions as result-changing in sensitivity analysis', () => {
    ...
    const result = solveNash(store.formalizations.formalization_1 as NormalFormModel, store)
    const sensitivity = analyzeSensitivity(store.formalizations.formalization_1 as NormalFormModel, result, store)

    expect(sensitivity.assumption_sensitivities.find((entry) => entry.assumption_id === 'assumption_1')?.impact).toBe('result_changes')
    expect(sensitivity.assumption_sensitivities.find((entry) => entry.assumption_id === 'assumption_2')?.impact).toBe('result_stable')
  })
  ...
  it('keeps sensitivity analysis bounded on larger games', () => {
    ...
    expect(sensitivity.threshold_analysis.every((entry) => entry.margin >= 0)).toBe(true)
  })
  ...
  it('returns backward-induction metadata immutably for weighted beliefs', () => {
    ...
    const result = solveBackwardInduction(store.formalizations.formalization_1 as ExtensiveFormModel, store)
