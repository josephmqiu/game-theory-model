...
import { getFormalizationAssumptionIds, getFormalizationEdges, getFormalizationNodes, readEstimateNumeric } from './utils'
...
function withEstimateValue(estimate: EstimateValue, value: number): EstimateValue {
  return {
    ...estimate,
    value,
  }
...
function buildNormalFormTarget(
  ...
  const linkedAssumptionIds = [...new Set([...formalization.assumptions, ...(estimate.assumptions ?? [])])]
...
function buildExtensiveFormTarget(
  ...
  const incomingEdges = getFormalizationEdges(canonical, formalization.id).filter((edge) => edge.to === nodeId)
  const linkedAssumptionIds = [...new Set([
  ...
  return {
    ...
    crossedMessage: () => `Changing terminal payoff at ${nodeLabel} flips the current solution path.`,
    buildStore: (nextValue) => ({
      ...canonical,
      nodes: {
        ...canonical.nodes,
  ...
  if (formalization.kind === 'extensive_form') {
    return getFormalizationNodes(canonical, formalization.id).flatMap((node) =>
      Object.entries(node.terminal_payoffs ?? {})
        .map(([playerId, estimate]) =>
          buildExtensiveFormTarget(canonical, formalization, node.id, node.label, playerId, estimate),
        )
        .filter((target): target is SensitivityTarget => target !== null),
    )
  }
...
export function analyzeSensitivity(
  ...
  const thresholdEvaluations = targets
    .map((target) => ({
      target,
      threshold: evaluateThreshold(target, formalization, solverResult, baselineFingerprint, targets.length),
    }))
    .sort((left, right) => left.threshold.margin - right.threshold.margin)

  const payoffSensitivities = thresholdEvaluations.map(({ target, threshold }) => ({
    ...
    direction: threshold.threshold >= target.currentValue ? 'increase' as const : 'decrease' as const,
    result_if_crossed: threshold.changed
      ? target.crossedMessage(threshold.threshold)
      : target.stableMessage,
  }))

  const thresholdAnalysis: ThresholdResult[] = thresholdEvaluations.map(({ threshold }) => ({
  ...
  const assumptionSensitivities: AssumptionSensitivity[] = assumptionIds.map((assumptionId) => {
    ...
    const changedEntries = linkedEntries.filter(({ threshold }) => threshold.changed)

    let description = 'No analyzed payoff targets were linked to this assumption.'
    if (linkedEntries.length > 0) {
      description = changedEntries.length > 0
    ...
    return {
      ...
      description,
      affected_payoffs: linkedEntries.map(({ threshold }) => threshold.parameter),
    }
  })
  ...
  const overallRobustness = payoffSensitivities.length === 0
    ? 'robust'
    : ratio > 0.2
      ? 'robust'
      : ratio >= 0.05
