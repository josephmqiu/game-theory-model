...
describe("debug tail sanitization", () => {
  it("should match ANTHROPIC_API_KEY leak", () => {
    expect(SENSITIVE_LOG_PATTERN.test("ANTHROPIC_API_KEY=sk-ant-abc123")).toBe(
      true,
    );
  });

  it("should match Authorization Bearer header", () => {
    expect(SENSITIVE_LOG_PATTERN.test("Authorization: Bearer token123")).toBe(
      true,
    );
  ...
  it("should NOT match normal log lines", () => {
    expect(
      SENSITIVE_LOG_PATTERN.test(
        "Using API endpoint https://api.anthropic.com",
      ),
    ).toBe(false);
    expect(
      SENSITIVE_LOG_PATTERN.test("Model: claude-sonnet-4-5-20250929"),
    ).toBe(false);
    expect(SENSITIVE_LOG_PATTERN.test("Request completed in 1200ms")).toBe(
      false,
    );
    expect(SENSITIVE_LOG_PATTERN.test("Connecting to upstream server...")).toBe(
      false,
    );
  });
});
