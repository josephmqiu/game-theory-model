...
const ALLOWED_FACT_CATEGORIES: FactCategory[] = [
  "capability",
  "economic",
  "position",
  "impact",
...
const PHASE_1_RELATIONSHIP_TYPES: RelationshipType[] = [
  "supports",
  "contradicts",
  "precedes",
];
...
describe("eval fixtures", () => {
  it("use only valid Phase 1 fact categories and relationship types", () => {
    ...
    for (const fixture of fixtures) {
      ...
      for (const type of expectations.allowedRelationshipTypes ?? []) {
        expect(PHASE_1_RELATIONSHIP_TYPES.includes(type)).toBe(true);
      }
  ...
  it("encode key regression guards for trivial, standard, and complex Phase 1 prompts", () => {
    ...
    const rps = byName.get("rps")?.phases["situational-grounding"];
    ...
    const union = byName.get("union-contract")?.phases["situational-grounding"];
    expect(union?.requiredFactCategories).toEqual(
      expect.arrayContaining(["position", "rule"]),
    );

    const semiconductor = byName.get("semiconductor-trade")?.phases[
