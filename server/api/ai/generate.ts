...
import { generateText as customGenerateText } from "../../services/ai/custom-openai-adapter";

interface CustomProviderRequest {
  baseURL: string;
  apiKey: string;
...
const generateBodySchema = z
  .object({
    ...
    provider: z.enum(["anthropic", "openai", "opencode", "custom"]),
...
export default defineEventHandler(async (event) => {
  ...
  if (bodyError) {
    ...
    if (!validatedBody) {
      ...
      result = await generateViaAgentSDK(
        validatedBody,
        validatedBody.model,
        runId,
      );
    } else if (validatedBody.provider === "opencode") {
      result = await generateViaOpenCode(
        validatedBody,
        validatedBody.model,
        runId,
      );
    } else if (validatedBody.provider === "openai") {
      result = await generateViaCodex(
        validatedBody,
        validatedBody.model,
        runId,
      );
    } else if (validatedBody.provider === "custom") {
      result = await generateViaCustom(
        validatedBody,
        validatedBody.model,
        runId,
      );
...
/** Generate via a custom OpenAI-compatible endpoint (non-streaming, no tools) */
async function generateViaCustom(
  ...
  const startedAt = Date.now();
  serverLog(runId, "generate", "custom-request-start", {
    model,
    systemLen: body.system.length,
    messageLen: body.message.length,
  });
  ...
  try {
    const text = await customGenerateText(
      {
        system: body.system,
        ...
        hasNativeWebSearch: body.custom.hasNativeWebSearch,
      },
      { runId },
    );
    serverLog(runId, "generate", "custom-request-complete", {
      elapsedMs: Date.now() - startedAt,
    ...
    const message = error instanceof Error ? error.message : String(error);
    serverError(runId, "generate", "custom-request-error", {
      elapsedMs: Date.now() - startedAt,
      error: message,
    });
    return { error: message };
    ...
    new Promise<{ data: null; error: string }>((resolve) =>
      setTimeout(
        () =>
          resolve({
            data: null,
            error: `OpenCode prompt timed out after ${timeoutMs / 1000}s`,
          }),
        timeoutMs,
      ),
    ),
...
async function generateViaOpenCode(
  ...
  try {
    ...
    const { data: result, error: promptError } =
      await promptOpenCodeWithThinking(ocClient, promptPayload, body, runId);
