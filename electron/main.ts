...
function setupIPC(): void {
  ...
  ipcMain.handle(
    ...
    async (_event, payload: { filePath: string; content: string }) => {
      ...
      if (!resolved.endsWith(".gta.json")) {
        throw new Error("Only .gta.json file extension is allowed");
      }
  ...
  ipcMain.handle("file:read", async (_event, filePath: string) => {
    ...
    if (!resolved.endsWith(".gta.json")) return null;
    const allowedRoots = [app.getPath("home"), app.getPath("temp")];
    const inAllowedDir = allowedRoots.some(
      (root) => resolved === root || resolved.startsWith(root + sep),
    );
  ...
  ipcMain.handle("prefs:set", (_event, key: string, value: string) => {
    ...
    if (key.length === 0 || key.length > 128) return;
  ...
  ipcMain.handle("prefs:remove", (_event, key: string) => {
    const { [key]: _removed, ...rest } = prefsCache;
    prefsCache = rest;
    schedulePrefsWrite();
  });
