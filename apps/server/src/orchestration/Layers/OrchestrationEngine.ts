...
type AnyCommand = OrchestrationCommand | AnalysisCommand;
...
function isAnalysisCommand(command: AnyCommand): command is AnalysisCommand {
  return (command.type as string).startsWith("analysis.");
}

function commandToAggregateRef(command: AnyCommand): {
  readonly aggregateKind: "project" | "thread" | "analysis";
  readonly aggregateId: ProjectId | ThreadId | AnalysisRunId;
} {
  if (isAnalysisCommand(command)) {
    // analysis.start doesn't have a runId yet; use commandId as a placeholder
    const runId =
      "runId" in command && typeof command.runId === "string"
        ? command.runId
        : command.commandId;
    return {
      aggregateKind: "analysis",
      aggregateId: runId as AnalysisRunId,
    };
  }
...
const makeOrchestrationEngine = Effect.gen(function* () {
  ...
  let analysisSlice: AnalysisReadModelSlice = createEmptyAnalysisSlice();
  ...
  const processEnvelope = (envelope: CommandEnvelope): Effect.Effect<void> => {
    ...
    const reconcileReadModelAfterDispatchFailure = Effect.gen(function* () {
      ...
      for (const persistedEvent of persistedEvents) {
        ...
        analysisSlice = projectAnalysisEvent(analysisSlice, persistedEvent);
      }
    ...
    return Effect.gen(function* () {
      ...
      // Route to the correct decider based on command type prefix
      const eventBase = isAnalysisCommand(envelope.command)
        ? yield* decideAnalysisCommand({
      ...
      const committedCommand = yield* sql
        .withTransaction(
          Effect.gen(function* () {
            ...
            let nextReadModel = readModel;
            let nextAnalysisSlice = analysisSlice;

            for (const nextEvent of eventBases) {
              const savedEvent = yield* eventStore.append(nextEvent);
              nextReadModel = yield* projectEvent(nextReadModel, savedEvent);
              nextAnalysisSlice = projectAnalysisEvent(
                nextAnalysisSlice,
                savedEvent,
              );
      ...
      analysisSlice = committedCommand.nextAnalysisSlice;
  ...
  yield* Stream.runForEach(eventStore.readAll(), (event) =>
    Effect.gen(function* () {
      ...
      analysisSlice = projectAnalysisEvent(analysisSlice, event);
    }),
  );
