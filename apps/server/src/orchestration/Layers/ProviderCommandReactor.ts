...
import { resolveThreadWorkspaceCwd } from "../../workspace-utils.ts";
...
const make = Effect.gen(function* () {
  ...
  const ensureSessionForThread = Effect.fnUntraced(function* (
    ...
    const threadProvider: ProviderKind =
      currentProvider ?? inferProviderForModel(thread.model);
    if (
    ...
    Effect.gen(function* () {
      switch (event.type) {
        case "thread.runtime-mode-set": {
          ...
          const cachedModelOptions = threadModelOptions.get(
            event.payload.threadId,
          );
