...
import type { AnalysisMutationEvent } from "./types/events";
import { serverLog } from "./logger-stub";
import * as runtimeStatus from "./runtime-status";
import * as persistence from "./entity-graph-persistence";
...
/**
 * Initialize SQLite write-through persistence for the entity graph.
 * Must be called AFTER migrations have run, BEFORE the server accepts requests.
 *
 * - Opens a synchronous SQLite connection to the same state.sqlite database
 * - Loads any previously persisted analysis state from disk
 * - All subsequent mutations write through to SQLite automatically
 */
export function initPersistence(dbPath: string): void {
  persistence.init(dbPath);

  // Hydrate in-memory state from disk
  const loaded = persistence.loadFromDisk();
  if (loaded) {
    analysis = normalizeAnalysis(loaded);
    _isDirty = false;
    console.log(
      `[entity-graph] Loaded ${loaded.entities.length} entities, ${loaded.relationships.length} relationships from disk (topic: "${loaded.topic}")`,
    );
  }
}
...
export function newAnalysis(topic: string): void {
  ...
  // Persist: clear old state and save the new empty analysis
  if (persistence.isInitialized()) {
    persistence.clearAnalysisState();
    persistence.saveAnalysisState(analysis);
  }
}
...
export function loadAnalysis(
  ...
  // Persist: replace all persisted state with the loaded analysis
  if (persistence.isInitialized()) {
    persistence.clearAnalysisState();
    persistence.saveAnalysisState(analysis);
    for (const entity of analysis.entities) {
      persistence.insertEntity(entity);
    }
    for (const rel of analysis.relationships) {
      persistence.insertRelationship(rel);
    }
  }
}
...
export function createRelationship(
  ...
  // Persist
  if (persistence.isInitialized()) {
    persistence.insertRelationship(relationship);
  }
...
export function updateEntity(
  ...
  // Persist
  if (persistence.isInitialized()) {
    persistence.updateEntityRow(id, updated);
  }
...
export function updateRelationship(
  ...
  // Persist: delete old and insert updated (simpler than column-by-column update)
  if (persistence.isInitialized()) {
    persistence.deleteRelationship(id);
    persistence.insertRelationship(updated);
  }
...
export function markStale(entityIds: string[]): void {
  ...
  // Persist
  if (persistence.isInitialized()) {
    persistence.updateEntityStale(entityIds, true);
  }
}
...
export function clearStale(entityIds: string[]): void {
  ...
  // Persist
  if (persistence.isInitialized()) {
    persistence.updateEntityStale(entityIds, false);
  }
}
...
export function removeEntity(id: string): boolean {
  ...
  // Persist (cascade-deletes relationships referencing this entity)
  if (persistence.isInitialized()) {
...
export function removePhaseEntities(
  ...
  // Persist
  if (persistence.isInitialized() && removedIds.size > 0) {
    persistence.deleteEntitiesByIds(Array.from(removedIds));
  }
}
