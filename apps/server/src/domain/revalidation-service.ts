...
import type { ResolvedAnalysisRuntime } from "./types/analysis-runtime";
import { V3_PHASES, PHASE_NUMBERS } from "./types/methodology";
...
import { createRunLogger, serverWarn, timer } from "./logger-stub";
...
function phasesFrom(startPhase: MethodologyPhase): MethodologyPhase[] {
  const startIdx = V3_PHASES.indexOf(startPhase);
  if (startIdx === -1) return [];
  return V3_PHASES.slice(startIdx);
}
...
async function executeRevalidation(
  ...
  for (const p of phases) {
    ...
    if (result.success) {
      ...
      const phaseEntityIds = entityGraphService
        .getEntitiesByPhase(p)
        .map((e) => e.id);
