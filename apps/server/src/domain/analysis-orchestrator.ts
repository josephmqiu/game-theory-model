...
import type { AnalysisProgressEvent, PhaseSummary } from "./types/events";
import type {
...
// commitPhaseSnapshot removed: entities are now created via CQRS tool calls
import {
  getRecordedLoopbackTriggers,
  clearRecordedLoopbackTriggers,
} from "./analysis-tools";
import type { LoopbackTriggerType } from "./analysis-tools";
import { analysisRuntimeConfig } from "./analysis-runtime-config";
import { resolveAnalysisRuntime } from "./analysis-runtime-resolver";
import * as runtimeStatus from "./runtime-status";
import { createRunLogger, timer } from "./logger-stub";
...
function emitPhaseActivity(
  ...
  options?: {
    kind?:
      | Extract<AnalysisProgressEvent, { type: "phase_activity" }>["kind"]
      | undefined;
...
async function executeSinglePhase(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (result.success) {
      // Entities are created via CQRS tool calls during the phase.
      // No batch commit needed — just record the summary.
...
export async function runFull(
  ...
  // activeRun was just assigned above so it's guaranteed non-null at this point.
  const run = activeRun!;
