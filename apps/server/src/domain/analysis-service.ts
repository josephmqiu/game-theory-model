...
import { createRunLogger } from "./logger-stub";
import type { RunLogger } from "./logger-stub";
...
export interface AnalysisAdapter {
  runAnalysisPhase<T = unknown>(
    ...
    options?: {
      ...
      onActivity?: AnalysisActivityCallback | undefined;
    },
  ): Promise<T>;
}

// Adapter loading removed — will be wired to T3 adapters in Step 4.
// Use setAnalysisAdapter() to inject the adapter before running phases.
let _injectedAdapter: AnalysisAdapter | null = null;
...
async function loadAnalysisAdapter(
  ...
  if (_injectedAdapter) {
    return _injectedAdapter;
  }
  throw new Error(
    "No analysis adapter configured. Call setAnalysisAdapter() before running phases.",
  );
}
...
export interface PhaseContext {
  revisionRetryInstruction?: string | undefined;
  revisionSystemPrompt?: string | undefined;
  provider?: string | undefined;
  model?: string | undefined;
  runtime?: PhaseRuntimeContext | undefined;
  runId?: string | undefined;
  signal?: AbortSignal | undefined;
  ...
  logger.log("analysis-service", "structured-output", {
    ...
    relationshipsCount:
      ...
      Array.isArray((adapterResult as Record<string, unknown>).relationships)
        ? ((adapterResult as Record<string, unknown[]>).relationships ?? [])
            .length
        : 0,
  });
