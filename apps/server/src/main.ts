...
import { initPersistence } from "./domain/entity-graph-service";
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const baseDir = yield* resolveBaseDir(
        Option.getOrUndefined(input.t3Home) ?? env.t3Home,
      );
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    // Initialize entity graph SQLite persistence (after migrations have run)
    yield* Effect.sync(() => initPersistence(config.dbPath));
