...
describe("WebSocket Server", () => {
  ...
  it("responds to server.getConfig", async () => {
    ...
    const { keybindingsConfigPath: keybindingsPath } = deriveServerPathsSync(
      baseDir,
      undefined,
    );
  ...
  it("catches websocket message handler rejections and keeps the socket usable", async () => {
    ...
    try {
      ...
      // Send a request with an unknown method tag to trigger a decode error
      ws.send(
        JSON.stringify({
          id: "req-broken",
          body: {
            _tag: "nonexistent.method",
          },
        }),
      );
