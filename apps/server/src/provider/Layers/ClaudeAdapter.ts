...
function buildPromptText(input: ProviderSendTurnInput): string {
  ...
  const promptEffort =
    ...
    supportsClaudeUltrathinkKeyword(input.model)
      ? "ultrathink"
...
function makeClaudeAdapter(options?: ClaudeAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const nextEventId = Effect.map(Random.nextUUIDv4, (id) =>
      EventId.makeUnsafe(id),
    );
    ...
    const backfillAssistantTextBlocksFromSnapshot = (
      ...
      Effect.gen(function* () {
        ...
        const orderedBlocks = turnState.assistantTextBlockOrder.map(
          (block) => ({
      ...
      Effect.gen(function* () {
        ...
        const effort =
          requestedEffort && supportedEffortOptions.includes(requestedEffort)
            ...
            : null;
        const fastMode =
          input.modelOptions?.claudeAgent?.fastMode === true &&
          supportsClaudeFastMode(input.model);
        const thinking =
