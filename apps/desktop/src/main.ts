...
// TODO: T3CODE_HOME env var should be renamed to GTA_HOME in a future pass
const BASE_DIR =
  process.env.T3CODE_HOME?.trim() || Path.join(OS.homedir(), ".gta");
const STATE_DIR = Path.join(BASE_DIR, "userdata");
const DESKTOP_SCHEME = "gta";
const ROOT_DIR = Path.resolve(__dirname, "../../..");
const isDevelopment = Boolean(process.env.VITE_DEV_SERVER_URL);
const APP_DISPLAY_NAME = isDevelopment
  ? "Game Theory Analyzer (Dev)"
  : "Game Theory Analyzer";
const APP_USER_MODEL_ID = "com.gta.game-theory-analyzer";
const USER_DATA_DIR_NAME = isDevelopment ? "gta-dev" : "gta";
// Keep legacy dir names so existing T3 Code users don't lose their Chromium profile data
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox(
      "Game Theory Analyzer failed to start",
      `Stage: ${stage}\n${message}${detail}`,
    );
  }
  stopBackend();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Game Theory Analyzer ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * package.json. We override it to a clean lowercase name (`gta`).
 * If the legacy T3 Code directory already exists we keep using it so
 * existing users don't lose their Chromium profile data (localStorage,
 * cookies, sessions).
 */
...
// TODO: Rename T3CODE_* env vars to GTA_* across the entire codebase (server reads these too)
