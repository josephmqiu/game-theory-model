...
function parseDetailStatus(line: string): {
  ...
  if (match)
    return { status: match[1] as "done" | "pending" | "error", text: match[2] };
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-foreground">
            {t("analysis.activity.agentProgress")}
          </span>
        </div>
          ...
          {items.map((item, index) => (
            ...
            >
              <div className="flex items-center gap-2 text-[11px] text-muted-foreground/90">
                ...
                <span className={cn(item.active ? "text-foreground" : "")}>
                  {item.label}
                </span>
              </div>
